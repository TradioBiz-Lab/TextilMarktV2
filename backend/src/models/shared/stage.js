...
// Extracted from Order.js so MfrProject can carry real TNA/stage tracking on a
// manufacturer's own non-Tradio projects, using the exact same stage machinery
// as a Tradio Order rather than a second, hand-rolled copy. Order.js re-exports
// every name below verbatim — this is a pure extraction, not a rename; nothing
// that already imports from '../models/Order.js' needs to change.

// One update note in a stage's ticket-style progress thread.
export const stageUpdateSchema = new mongoose.Schema({
...
// all of which shift array positions elsewhere.
export const stageMaterialSchema = new mongoose.Schema({
  name:         { type: String, required: true, maxlength: 200 },
  // Planning metadata for the Materials Management module — deliberately NOT read
  // by the production gate (isTrims/isTrimsOrderStage stay name-keyed only, see
  // routes/orders.js), so this never changes existing gating behavior.
...
export const stageItemSchema = new mongoose.Schema({
  name:      { type: String, required: true, maxlength: 200 },
...
export const stageSchema = new mongoose.Schema({
  ...
  startDate:  { type: String, default: null }, // ISO date string or 'NA' — required at creation, null only on legacy pre-existing data
  ...
  // see the gating check in routes/orders.js (and routes/mfrProjectStages.js).
  materials:  [stageMaterialSchema],
  items:      [stageItemSchema],
}, { _id: false })

// ── Stage derivation helpers ────────────────────────────────────────────────
// Every order/project read is .lean(), so Mongoose never applies schema defaults
// to documents written before a field existed — those come back undefined. These
// helpers are the single place that resolves that, and enrichOrder/enrichMfrProject
// are the only serialization points, so normalizing there replaces a migration.
 ...
 * already validated — for "mark it done, it actually finished on the 3rd"
 * narration. Takes priority over both the preserved stamp and today's date.
 */
