...
const TOOLS = [
  ...
  },
  {
    ...
    description: "Change a stage's progress. The body shape depends on the stage's `kind` — fetch it first via get_order/list_orders. For a `quantity`-kind stage: to close it outright, send `status: 'done'` (units auto-fill to the total — do not also compute/send `unitsDone` for a close); to log partial progress without closing, send `unitsDone` (count completed) instead and status is derived automatically. For `milestone`/`checklist`-kind stages, set `status` directly; do not send `unitsDone`. `note` optionally updates the stage's transient note. `blocked`/`blockedReason` flag or clear a blocker independent of status. When marking a stage `done` and the admin states it actually finished on a different (past) date than today, pass that as `actualEnd` — otherwise it auto-stamps today. A stage with pending materials, or a checklist stage with pending items, cannot advance/close — the call fails with a descriptive error; report it back rather than retrying. Only set `override: true` if the admin explicitly asks to force it through past a pending-materials block — this only works for the master admin and is rejected otherwise.",
    input_schema: {
      type: 'object',
      properties: {
        orderId: { type: 'string' },
...
LANGUAGE: you are fully fluent in Hindi, English, Kannada, and Bengali — including natural spoken conversation in each, not just translation. ${user.name} may talk to you by voice in any of these four languages. Always reply in the same language the admin just wrote or spoke in. If asked whether you can speak Hindi, Kannada, or Bengali, say yes plainly — never claim you only know English. Never open with a religious, regional, or culturally/community-specific greeting (e.g. "Assalam Aleikum", "Sat Sri Akal") in any language, even one commonly heard in that language's region — you don't know ${user.name}'s community and guessing one is presumptuous. If a greeting is needed at all, keep it plain and neutral (Hindi: "Namaste" or none; Bengali: "Nomoshkar" or none; Kannada: "Namaskara" or none; English: none needed) — and only echo back a specific greeting ${user.name} used first, never introduce one unprompted.
